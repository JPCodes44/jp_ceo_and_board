...
test("detectExplicitAgentRole honors explicit team member requests", () => {
  assert.equal(detectExplicitAgentRole("use the reviewer for this"), "Reviewer");
  assert.equal(detectExplicitAgentRole("builder should do this change"), "Builder");
  assert.equal(detectExplicitAgentRole("delegate this to red team"), "Red Team");
});

test("routeTextToAgentRole picks the best matching team member", () => {
  assert.deepEqual(routeTextToAgentRole("find the auth flow and map the main files"), {
    role: "Scout",
    profile: "scout",
    explicit: false,
  ...
  assert.deepEqual(routeTextToAgentRole("document the API and update the readme"), {
    role: "Documenter",
    profile: "documenter",
    explicit: false,
...
test("routeTextToAgentRole respects the active team roster", () => {
  assert.deepEqual(routeTextToAgentRole("what should we do next", ["Planner", "Builder"]), {
    role: "Planner",
    profile: "planner",
    explicit: false,
  ...
  assert.deepEqual(routeTextToAgentRole("use the reviewer for this", ["Builder"]), {
    role: "Builder",
    profile: "worker",
    explicit: false,
    reason: "Reviewer is not part of the active team, so routing used the current team roster.",
  });
});

test("parseAgentTeamsConfig supports inline and multiline team definitions", () => {
  assert.deepEqual(parseAgentTeamsConfig(`teams:
...
test("setActiveAgentTeam and setAgentTeamMemberModels update the active team", () => {
  ...
  setAgentTeamMemberModels(["Planner", "Builder", "Reviewer"], "gemini-2.5-pro");
...
test("assignTaskToAgentTeam marks the selected member as assigned", () => {
  resetAgentTeamRuntimeState();
  const decision = routeTextToAgentRole("use the reviewer to audit this change");
  assignTaskToAgentTeam("use the reviewer to audit this change", decision);

  const state = getAgentTeamStateSnapshot();
  assert.equal(state.members.Reviewer.status, "assigned");
  assert.equal(state.members.Reviewer.lastTask, "use the reviewer to audit this change");
...
test("detectRequestedAgentRoles finds multiple explicit agents and full-team broadcasts", () => {
  ...
  assert.deepEqual(
    detectRequestedAgentRoles("run the agent team on this"),
...
test("assignTaskToAgentTeam keeps multiple requested members assigned", () => {
  resetAgentTeamRuntimeState();
  const decision = routeTextToAgentRole("ask the builder and reviewer to handle this");
  assignTaskToAgentTeam("ask the builder and reviewer to handle this", decision, {
...
test("resolveAgentTeamSubagentRoute rotates through requested team members", () => {
  ...
  const first = resolveAgentTeamSubagentRoute(undefined, "ask the builder and reviewer to handle this");
  assert.equal(first.role, "Builder");
  assert.equal(first.profile, "worker");

  assignTaskToAgentTeam("ask the builder and reviewer to handle this", first, { preserveRequestedRoles: true });
  const second = resolveAgentTeamSubagentRoute(undefined, "ask the builder and reviewer to handle this");
...
test("team runtime captures thinking blurbs and full logs", () => {
  ...
  assert.equal(state.members.Planner.thinkingLog, "First pass on tradeoffs\nSecond pass on sequencing");
});
...
test("routeTextToAgentRole respects direct multi-agent addressing", () => {
  assert.deepEqual(routeTextToAgentRole("hey scout and planner look thruy hte codebase"), {
    role: "Scout",
    profile: "scout",
    explicit: true,
