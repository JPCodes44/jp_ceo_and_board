...
test('resolveSubagentTools keeps non-worker profiles read-only', () => {
  assert.deepEqual(resolveSubagentTools('general'), [
    'read',
    'grep',
    'find',
    'ls',
  ]);
  assert.deepEqual(resolveSubagentTools('scout'), [
    'read',
    'grep',
    'find',
    'ls',
  ]);
  assert.deepEqual(resolveSubagentTools('planner'), [
    'read',
    'grep',
    'find',
    'ls',
  ]);
  assert.deepEqual(resolveSubagentTools('reviewer'), [
    'read',
    'grep',
    'find',
    'ls',
  ]);
  assert.deepEqual(resolveSubagentTools('documenter'), [
    'read',
    'grep',
    'find',
    'ls',
  ]);
  assert.deepEqual(resolveSubagentTools('red-team'), [
    'read',
    'grep',
    'find',
    'ls',
...
test('resolveSubagentTools enables worker write tools only when requested', () => {
  assert.deepEqual(resolveSubagentTools('worker', false), [
    'read',
    'grep',
    'find',
    'ls',
  ]);
  assert.deepEqual(resolveSubagentTools('worker', true), [
    'read',
    'bash',
    'edit',
    'write',
...
test('buildSubagentPrompt reflects read-only and write-capable modes', () => {
  assert.match(
    buildSubagentPrompt({ agent: 'general', allowWrite: false }),
    /read-only mode/i,
  );
  assert.match(
    buildSubagentPrompt({ agent: 'worker', allowWrite: true }),
    /mutation tools/i,
  );
});
...
test('tokenizeSubagentCommand preserves quoted segments', () => {
  assert.deepEqual(
    tokenizeSubagentCommand('worker --model sonnet "fix the failing test"'),
    ['worker', '--model', 'sonnet', 'fix the failing test'],
  );
});

test('parseSubagentCommand parses flags and task', () => {
  const result = parseSubagentCommand(
    'worker --write --model sonnet --thinking low "fix the failing test"',
  );
...
test('parseSubagentCommand treats unknown first args as general prompt text', () => {
  ...
  if (result.ok !== true) {
    throw new Error(
      'parseSubagentCommand unexpectedly failed for freeform input',
    );
  }
...
test('parseSubagentCommand allows flags without an explicit agent', () => {
  const result = parseSubagentCommand(
    '--model sonnet --thinking low summarize the auth flow',
  );

  assert.equal(result.ok, true);
  if (result.ok !== true) {
    throw new Error(
      'parseSubagentCommand unexpectedly failed for general-with-flags input',
    );
  }
...
test('parseSubagentKillCommand defaults to the top subagent when no args are provided', () => {
  ...
  if (result.ok !== true) {
    throw new Error(
      'parseSubagentKillCommand unexpectedly failed for empty input',
    );
  }
...
test('getSubagentPreviewText prefers latest stderr output', () => {
  const stdoutPreview = appendSubagentOutputPreview(
    createEmptySubagentOutputPreview(),
    'mapped files\nnext step',
  );
  const stderrPreview = appendSubagentOutputPreview(
    createEmptySubagentOutputPreview(),
    'warning\n',
  );
...
test('buildSubagentProgressLines renders a compact live widget', () => {
  ...
  assert.deepEqual(
    ...
    ['Subagent #3 fix failing tests (1m 05s) | Tools: 7', 'warning'],
  );
});
