...
import { getNextThemeName, getPreferredThemeNames } from "../.pi/lib/theme-cycle.ts";

test("getPreferredThemeNames keeps the requested theme order only", () => {
  assert.deepEqual(
    getPreferredThemeNames(["dark", "green", "white", "orange", "solarized", "default"]),
...
test("getNextThemeName falls back to the first preferred theme when current is missing", () => {
  assert.equal(getNextThemeName(["white", "green", "default"], undefined), "green");
  assert.equal(getNextThemeName(["white", "green", "default"], "missing"), "green");
});

test("getNextThemeName cycles in the requested custom order and wraps around", () => {
  const themeNames = ["white", "cyan", "green", "default", "black", "orange", "purple"];
